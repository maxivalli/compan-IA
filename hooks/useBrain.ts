...
export function useBrain(deps: BrainDeps) {
  ...
  async function responderConClaude(textoUsuario: string, prebuiltTurnId?: string) {
    ...
    if (pideBusqueda) {
      ...
      if (esConsultaDistancia) {
        // Extraer el destino de frases como "hasta Rosario", "llegar a Buenos Aires", etc.
        const destMatch = textoUsuario.match(/\b(?:hasta|para llegar a|llegar a|de aca a|de aqui a|llego a)\s+([A-ZÁÉÍÓÚÑa-záéíóúñü][a-záéíóúñü\s]{1,25}?)(?:\s*[?,]|\s*$)/i);
        const destino = destMatch?.[1]?.trim();
        queryBusqueda = destino && ciudad
          ? `distancia de ${ciudad} a ${destino} Argentina kilómetros`
          : ciudad
          ? `${textoUsuario} desde ${ciudad} Argentina kilómetros`
          : `${textoUsuario} Argentina distancia kilómetros`;
      } else if (esTelefono && ciudad)        queryBusqueda = `${queryBusqueda} número de teléfono ${ciudad} Argentina`;
