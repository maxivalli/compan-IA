...
export function useBrain(deps: BrainDeps) {
  ...
  async function responderConClaude(textoUsuario: string, prebuiltTurnId?: string) {
    ...
    const esConsultaDistancia = /\b(cuanto hay (hasta|de .{1,20} a)|cuanto queda|cuantos kil|cuantos km|distancia (hasta|a |de |entre)|a cuantos (km|kil)|que tan lejos|lejos (queda|esta|hay)|cuanto (se tarda|tarda|tardaria|demora|demoraria) (en llegar|hasta)|para llegar a|kilometros (hasta|a |de ))\b/.test(textoNorm);
